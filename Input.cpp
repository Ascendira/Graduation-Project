...
void LoadInput(InputCondition *ICond, InputMaterial *IMaterial, InputProperty *IProp)
{
        ...
        IMaterial->D[3] = (1.43E-7) * exp(-1.91 / (kb * ICond->Temp));        // eV 转为 J/mol 或直接用 kb 处理
        ...
        IMaterial->X[0][3] = 0.0;       // Cu composition in T3/G (Mn6Ni16Si7) phase
        ...
        IMaterial->X[1][3] = 0.0;     // Cu composition in T6/Gamma2 (Mn(Ni,Si)2) phase

        IMaterial->X[2][0] = 0.0; // Mn composition in Cu-rich (Cu2MnAl) phase
        IMaterial->X[2][1] = 0.0; // Ni composition in Cu-rich (Cu2MnAl) phase
        IMaterial->X[2][2] = 0.0; // Si composition in Cu-rich (Cu2MnAl) phase
        IMaterial->X[2][3] = 1.0; // Cu composition in Cu-rich (Cu2MnAl) phase
        ...
        IMaterial->sig[2] = 0.420 * 6.241E18; // Cu-rich phase interfacial energy is 0.42J/m2
        ...
        IMaterial->cVol[2] = IMaterial->aVol;         // Atomic volume of Cu-rich phase, m^-3
        ...
        IMaterial->solPBar[2] = exp(1.0) * exp(-0.545 / (kb * ICond->Temp)); // Equilibrium solute product of Cu-rich phase at 573K
        //*******************************************
